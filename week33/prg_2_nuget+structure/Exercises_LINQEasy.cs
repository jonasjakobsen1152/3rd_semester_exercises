...
public class EasyLinqExercises : IEasyLinqExercises
{
    ...
    {
        List<int> first3Numbers = new List<int>();
        first3Numbers = numbers.Slice(0, 3);
    ...
    {
        var helloString = from i in names select $"Hello, {i}";
        return helloString.ToList();
    }
    ...
    {
        var toBeDoubled = from i in numbers select i * 2;
        return toBeDoubled.ToList();
    }

    // Exercise 8: Return a list of strings converted to uppercase.
    public List<string> ConvertToUpper(List<string> words)
    {
        var toBeConverted = from i in words select i.ToUpper();
        return toBeConverted.ToList();
    }
